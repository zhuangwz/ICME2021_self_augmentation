...
tf.flags.DEFINE_integer('image_resize', 331, 'Height of resize image.')
tf.flags.DEFINE_boolean('use_dev', True, 'use deviation augmentation or not.')
tf.flags.DEFINE_boolean('use_self', True, 'use self ensemble or not.')
tf.flags.DEFINE_integer('sample_num', 4, 'the number of noise if use_dev is true.')
tf.flags.DEFINE_float('sample_variance', 0.12, 'the variance of noise if use_dev is true.')
tf.flags.DEFINE_string('checkpoint_path', '/ckpt_models/',
                       'Path to checkpoint for pretained models.')
tf.flags.DEFINE_string('input_dir', '/image_net/',
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    logits_v4, end_points_v4 = inceptionv4_model(x_dev)
    logits_v2, end_points_v2 = inceptionresnetv2_model(x_dev)
    ...
    # SI-TIM
    x_nes_2 = 1 / 2 * x_dev
    logits_v3_2, end_points_v3_2 = inceptionv3_model(x_nes_2)
    ...
    # self-ensemble
    if FLAGS.use_self:
        x_conv_9 = tf.nn.depthwise_conv2d(x_dev, stack_kernel_9, strides=[1, 1, 1, 1], padding='SAME')
        logits_v3_9, end_points_v3_9 = inceptionv3_model(x_conv_9)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        y = tf.one_hot(predicted_labels, 1001)
